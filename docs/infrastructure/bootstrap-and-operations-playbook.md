...
> ⚠ **Known gap (phase 1 gate blocker).** Only `event-store/migrations/` is auto-applied today. The compose `postgres` service mounts `../data/postgres/event-store/migrations:/docker-entrypoint-initdb.d`, and [migrate.sh](../../infrastructure/deployment/scripts/migrate.sh) only iterates that same directory. **Outbox, chain, and HSID migrations are not currently applied by bootstrap.** First-run hosts crash with `HSID FATAL` until the HSID migration is applied by hand. Tracked by CHECK-INFRA-TOPICS-01 + CHECK-INFRA-HSID-INITDB-01 in [infrastructure.audit.md](../../claude/audits/infrastructure.audit.md). Until this is fixed, after bootstrap run:
> ```bash
