...
/// <see cref="EnforcementToCapitalIntegrationHandler"/> so that detected
/// violations with a Block recommendation result in a
/// <c>FreezeCapitalAccountCommand</c> dispatched against the identified
/// subject. Mirrors <see cref="LedgerToCapitalIntegrationWorker"/> and
/// <see cref="EnforcementToPolicyFeedbackWorker"/> in shape so malformed-message
/// handling and offset semantics are identical across integration workers.
///
/// Subscribed topic: whyce.economic.enforcement.violation.events
/// Consumer group:   whyce.integration.enforcement-to-capital
///
/// Failure handling: malformed headers/payloads are skipped with a warning
/// and the offset IS committed (the projection worker DLQs malformed
/// messages). Handler exceptions leave the offset un-committed so the
/// message is redelivered; replay safety is owned by the handler's
/// envelope-level idempotency claim keyed on
/// <c>enforcement-to-capital:{EventId}</c>.
/// </summary>
public sealed class EnforcementToCapitalWorker : BackgroundService
{
    private const string SourceTopic = "whyce.economic.enforcement.violation.events";
    private const string ConsumerGroup = "whyce.integration.enforcement-to-capital";
    private const string WorkerName = "enforcement-to-capital"; // R2.E.1 rebalance-metric tag

    private readonly string _kafkaBootstrapServers;
    private readonly EventDeserializer _deserializer;
    private readonly EnforcementToCapitalIntegrationHandler _handler;
    private readonly IClock _clock;
    private readonly KafkaConsumerOptions _consumerOptions;
    private readonly ILogger<EnforcementToCapitalWorker>? _logger;
    ...
    public EnforcementToCapitalWorker(
        string kafkaBootstrapServers,
        EventDeserializer deserializer,
        EnforcementToCapitalIntegrationHandler handler,
        IClock clock,
        KafkaConsumerOptions consumerOptions,
        ILogger<EnforcementToCapitalWorker>? logger = null,
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        KafkaRebalanceObservability.Attach(consumerBuilder, SourceTopic, WorkerName, _logger);
        using var consumer = consumerBuilder.Build();
        consumer.Subscribe(SourceTopic);

        _logger?.LogInformation(
            "EnforcementToCapitalWorker subscribed to {Topic} under consumer group {Group}.",
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                KafkaLagObservability.Record(consumer, result, WorkerName, SourceTopic);
            ...
            {
                _logger?.LogError(ex, "EnforcementToCapitalWorker iteration failed outside the retry-escalate path; will continue.");
            }
        }
        ...
        CancellationToken ct)
    {
            ...
            || _randomProvider is null || _kafkaBreaker is null)
        {
            _logger?.LogError(handlerEx,
                "EnforcementToCapitalWorker handler failed on {Topic} p{Partition} o{Offset}; retry tier not wired, will redeliver.",
        ...
        {
            _logger?.LogWarning(
                "EnforcementToCapitalWorker handler failed on {Topic} p{Partition} o{Offset}; retry-escalate blocked by breaker '{Breaker}' ({RetryAfter}s); will redeliver.",
                result.Topic, result.Partition.Value, result.Offset.Value,
        ...
        catch (Exception escalationEx)
        {
            _logger?.LogError(escalationEx,
                "EnforcementToCapitalWorker retry escalation failed on {Topic} p{Partition} o{Offset} after handler exception {HandlerException}; will redeliver.",
