...
namespace Whycespace.Platform.Api.Controllers.Economic.Capital.Shared;

// Shared helper base for the seven capital domain controllers
// (account, allocation, asset, binding, pool, reserve, vault).
// Encapsulates the two cross-cutting concerns those controllers share:
//   - Dispatching commands via ISystemIntentDispatcher with a consistent
//     ack/failure response shape.
//   - Loading a JSON-state read-model row from the projections DB by
//     aggregate_id with a consistent not-found shape.
public abstract class CapitalControllerBase : ControllerBase
{
    ...
    protected CapitalControllerBase(
