...
public sealed class CreditSliceHandler : IEngine
{
    public async Task ExecuteAsync(IEngineContext context)
    {
        if (context.Command is not CreditSliceCommand cmd)
            return;

        var aggregate = (VaultAccountAggregate)await context.LoadAggregateAsync(typeof(VaultAccountAggregate));
        aggregate.CreditSlice(MapSlice(cmd.Slice), new Amount(cmd.Amount));
