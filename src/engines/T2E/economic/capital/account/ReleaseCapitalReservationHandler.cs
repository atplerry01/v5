...
public sealed class ReleaseCapitalReservationHandler : IEngine
{
    public async Task ExecuteAsync(IEngineContext context)
    {
        if (context.Command is not ReleaseCapitalReservationCommand cmd)
            return;

        var aggregate = (CapitalAccountAggregate)await context.LoadAggregateAsync(typeof(CapitalAccountAggregate));
        aggregate.ReleaseReservation(new Amount(cmd.Amount), new Currency(cmd.Currency));
