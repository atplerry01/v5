...
public sealed class ReserveCapitalAccountHandler : IEngine
{
    public async Task ExecuteAsync(IEngineContext context)
    {
        if (context.Command is not ReserveCapitalAccountCommand cmd)
            return;

        var aggregate = (CapitalAccountAggregate)await context.LoadAggregateAsync(typeof(CapitalAccountAggregate));
        aggregate.Reserve(new Amount(cmd.Amount), new Currency(cmd.Currency));
