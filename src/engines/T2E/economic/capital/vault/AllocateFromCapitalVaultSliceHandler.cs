...
public sealed class AllocateFromCapitalVaultSliceHandler : IEngine
{
    public async Task ExecuteAsync(IEngineContext context)
    {
        if (context.Command is not AllocateFromCapitalVaultSliceCommand cmd)
            return;

        var aggregate = (VaultAggregate)await context.LoadAggregateAsync(typeof(VaultAggregate));
        aggregate.AllocateFromSlice(new SliceId(cmd.SliceId), new Amount(cmd.Amount));
