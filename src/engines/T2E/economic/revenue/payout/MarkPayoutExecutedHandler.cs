...
public sealed class MarkPayoutExecutedHandler : IEngine
{
    private readonly IClock _clock;

    public MarkPayoutExecutedHandler(IClock clock) => _clock = clock;

    public async Task ExecuteAsync(IEngineContext context)
    {
        if (context.Command is not MarkPayoutExecutedCommand)
            return;

        var aggregate = (PayoutAggregate)await context.LoadAggregateAsync(typeof(PayoutAggregate));
        aggregate.MarkExecuted(new Timestamp(_clock.UtcNow));
