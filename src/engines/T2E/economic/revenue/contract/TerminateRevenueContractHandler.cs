...
public sealed class TerminateRevenueContractHandler : IEngine
{
    private readonly IClock _clock;

    public TerminateRevenueContractHandler(IClock clock) => _clock = clock;

    public async Task ExecuteAsync(IEngineContext context)
    {
        if (context.Command is not TerminateRevenueContractCommand cmd)
            return;

        var aggregate = (RevenueContractAggregate)await context.LoadAggregateAsync(typeof(RevenueContractAggregate));
        aggregate.Terminate(cmd.Reason, new Timestamp(_clock.UtcNow));
