...
public sealed class RemoveProfileDescriptorHandler : IEngine
{
    public async Task ExecuteAsync(IEngineContext context)
    {
        if (context.Command is not RemoveProfileDescriptorCommand cmd)
            return;

        var aggregate = (ProfileAggregate)await context.LoadAggregateAsync(typeof(ProfileAggregate));
        aggregate.RemoveDescriptor(cmd.Key);
