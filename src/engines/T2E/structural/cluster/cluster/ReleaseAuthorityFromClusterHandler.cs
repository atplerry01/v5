...
public sealed class ReleaseAuthorityFromClusterHandler : IEngine
{
    public async Task ExecuteAsync(IEngineContext context)
    {
        if (context.Command is not ReleaseAuthorityFromClusterCommand cmd) return;
        var aggregate = (ClusterAggregate)await context.LoadAggregateAsync(typeof(ClusterAggregate));
        aggregate.RecordAuthorityReleased(new ClusterAuthorityRef(cmd.AuthorityId));
