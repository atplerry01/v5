...
public sealed class BindAuthorityToClusterHandler : IEngine
{
    public async Task ExecuteAsync(IEngineContext context)
    {
        if (context.Command is not BindAuthorityToClusterCommand cmd) return;
        var aggregate = (ClusterAggregate)await context.LoadAggregateAsync(typeof(ClusterAggregate));
        aggregate.RecordAuthorityAttached(new ClusterAuthorityRef(cmd.AuthorityId));
