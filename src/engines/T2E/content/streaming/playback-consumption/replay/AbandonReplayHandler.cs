...
public sealed class AbandonReplayHandler : IEngine
{
    public async Task ExecuteAsync(IEngineContext context)
    {
        if (context.Command is not AbandonReplayCommand cmd) return;
        var aggregate = (ReplayAggregate)await context.LoadAggregateAsync(typeof(ReplayAggregate));
        aggregate.Abandon(new Timestamp(cmd.AbandonedAt));
