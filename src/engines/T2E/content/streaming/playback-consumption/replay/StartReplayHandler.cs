...
public sealed class StartReplayHandler : IEngine
{
    public async Task ExecuteAsync(IEngineContext context)
    {
        if (context.Command is not StartReplayCommand cmd) return;
        var aggregate = (ReplayAggregate)await context.LoadAggregateAsync(typeof(ReplayAggregate));
        aggregate.Start(new PlaybackPosition(cmd.PositionMs), new Timestamp(cmd.StartedAt));
