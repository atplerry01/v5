...
public sealed class MarkRetentionEligibleForDestructionHandler : IEngine
{
    public async Task ExecuteAsync(IEngineContext context)
    {
        if (context.Command is not MarkRetentionEligibleForDestructionCommand cmd)
            return;

        var aggregate = (RetentionAggregate)await context.LoadAggregateAsync(typeof(RetentionAggregate));
        aggregate.MarkEligibleForDestruction(new Timestamp(cmd.MarkedAt));
