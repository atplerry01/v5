...
public sealed class AssetAggregate : AggregateRoot
{
    ...
    public void AssignKind(AssetKind newKind, Timestamp assignedAt)
    {
        if (Status == AssetStatus.Retired)
            throw AssetErrors.CannotAssignKindToRetiredAsset();

        if (Kind == newKind)
            throw AssetErrors.AssetKindUnchanged();

        RaiseDomainEvent(new AssetKindAssignedEvent(AssetId, Kind, newKind, assignedAt));
    }
