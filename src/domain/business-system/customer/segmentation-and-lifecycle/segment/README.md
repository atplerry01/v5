# Segment

**Classification:** business-system
**Context:** customer
**Domain-Group:** segmentation-and-lifecycle
**Domain:** segment
**Namespace:** `Whycespace.Domain.BusinessSystem.Customer.SegmentationAndLifecycle.Segment`

## Responsibility
Business segment classification a customer belongs to.

## Canonical Artifact Layout (DS-R3a)
- `aggregate/` — root aggregate
- `entity/` — domain entities
- `error/` — domain-specific exceptions
- `event/` — domain events
- `service/` — domain services
- `specification/` — business-rule specifications
- `value-object/` — value objects

## Status
S4 — E1 domain model implemented (Batch 4). Aggregate + VOs + events + specs + errors.

## Routing
Per DS-R8, `DomainRoute` remains the 3-tuple `(business-system, customer, segment)`; the domain-group is a folder-level grouping and is not part of the route.
