...
public sealed class WorkflowTransitionCreateEngine : IEngine<CreateWorkflowTransitionCommand>
{
    private readonly IWorkflowDomainService _workflowDomainService;

    public WorkflowTransitionCreateEngine(IWorkflowDomainService workflowDomainService)
    {
        _workflowDomainService = workflowDomainService;
    }

    public async Task<EngineResult> ExecuteAsync(
        CreateWorkflowTransitionCommand command,
        ...
        CancellationToken ct)
    {
        ...
        if (!validation.IsValid)
            return EngineResult.Fail(validation.Reason ?? "Validation failed", "WORKFLOW_TRANSITION_CREATE_INVALID");
        ...
        var result = await _workflowDomainService.CreateTransitionAsync(execCtx, command.Id);
