...
public sealed class WorkflowStepCreateEngine : IEngine<CreateWorkflowStepCommand>
{
    private readonly IWorkflowDomainService _workflowDomainService;

    public WorkflowStepCreateEngine(IWorkflowDomainService workflowDomainService)
    {
        _workflowDomainService = workflowDomainService;
    }

    public async Task<EngineResult> ExecuteAsync(
        CreateWorkflowStepCommand command,
        ...
        CancellationToken ct)
    {
        ...
        if (!validation.IsValid)
            return EngineResult.Fail(validation.Reason ?? "Validation failed", "WORKFLOW_STEP_CREATE_INVALID");
        ...
        var result = await _workflowDomainService.CreateStepAsync(execCtx, command.Id);
