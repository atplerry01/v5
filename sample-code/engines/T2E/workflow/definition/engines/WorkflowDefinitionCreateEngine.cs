...
public sealed class WorkflowDefinitionCreateEngine : IEngine<CreateWorkflowDefinitionCommand>
{
    private readonly IWorkflowDomainService _workflowDomainService;

    public WorkflowDefinitionCreateEngine(IWorkflowDomainService workflowDomainService)
    {
        ...
        CancellationToken ct)
    {
        ...
        if (!validation.IsValid)
            return EngineResult.Fail(validation.Reason ?? "Validation failed", "WORKFLOW_DEFINITION_CREATE_INVALID");
        ...
        var result = await _workflowDomainService.CreateDefinitionAsync(execCtx, command.Id);
