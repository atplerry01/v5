...
{
    /// <summary>
    /// Resolves economic context from the command payload.
    /// Returns null if the command is not an economic command.
    /// </summary>
    public IEconomicContext? Resolve(CommandContext context)
    {
        ArgumentNullException.ThrowIfNull(context);

        // Check if this is an economic command by type prefix
        var commandType = context.Envelope.CommandType;
        if (!IsEconomicCommand(commandType))
            return null;

        // Extract economic fields from payload
...
/// <summary>
/// Concrete implementation of IEconomicContext resolved from command payload.
/// </summary>
